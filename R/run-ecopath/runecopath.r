...
DC=matrix(unlist(ecopathinput[1:n,11:(10+n)]),ncol=n)
Y=as.matrix(ecopathinput$Yield)
Grouptype=as.matrix(ecopathinput$Type)
Hg.E=as.matrix(ecopathinput$Hg_E)
Hg.ppm=as.matrix(ecopathinput$Hg_ppm)

livinggroups=which(Grouptype>0)
nliving=length(livinggroups)
consumers=which(Grouptype==2)
detrital_groups=which(Grouptype==0)
...
# Calculate total losses andgains for each group
...
for (j in 1:n) {
   ...
   CC[j,1]=sum(matrix(DC[j,],n,1)*QB*Bout)
}
...
GCE=PB/QB
GCE[Groupindex]=NaN
ecopathout=list(Bout=Bout, PB=PB, CC=CC, EEout=EEout, QB=QB, DC=DC, BA=BA, Mpmat=Mpmat, Mo=Mo, GCE=GCE,TL=TL, FF=FF, Hg.E=Hg.E, Hg.ppm=Hg.ppm, Grouptype=Grouptype)
return (ecopathout)
}  #end function
